...
# priority experience replay constants
PRIO_REPLAY_ALPHA = 0.6
BETA_START = 0.4
BETA_FRAMES = 100000

# categorical algorithm 51 atoms distribution constants
Vmax = 10
Vmin = -10
N_ATOMS = 51
...
class PongRainbowDQN(nn.Module):
    def __init__(self, input_shape, n_actions):
        super(PongRainbowDQN, self).__init__()
        ...
        self.fc_val = nn.Sequential(
            atari_deep_q_neural_network.NoisyLinear(conv_out_size, 512),
            nn.ReLU(),
            atari_deep_q_neural_network.NoisyLinear(512, N_ATOMS)
        )

        self.fc_adv = nn.Sequential(
            atari_deep_q_neural_network.NoisyLinear(conv_out_size, 512),
            nn.ReLU(),
            atari_deep_q_neural_network.NoisyLinear(512, n_actions * N_ATOMS)
        )
...
def calc_loss(batch, batch_weights, net, tgt_net, gamma, device="cpu"):
    states, actions, rewards, dones, next_states = common_utils.unpack_batch(batch)
    ...
    # dueling architecture -- actions from main net, distributions from target net

    # calc at once both next and current states
    ...
    # project distribution using bellman update
    proj_distr = common_utils.categorical_algorithm(next_best_distr, rewards, dones, Vmin, Vmax, N_ATOMS, gamma)
...
if __name__ == "__main__":
    ...
    net = PongRainbowDQN(env.observation_space.shape, env.action_space.n).to(device)
    ...
    with common_utils.RewardTracker(writer, params['stop_reward']) as reward_tracker:
        while True:
            frame_idx += 1
            buffer.populate(1)
